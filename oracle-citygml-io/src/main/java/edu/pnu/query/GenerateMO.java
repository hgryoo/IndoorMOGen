...
public class GenerateMO {
	...
	private static STPoint addNoise(SqlSession session, CommonGeometryFactory gf, STPoint orginP, final double sigma, final int chooseCount){
		...
		try {
			int beforeCellId = queryModule.intersectRoomByPoint(session, "Real Data", orginP);
			int afterCellId =  queryModule.intersectRoomByPoint(session, "Real Data", newPoint);
			for(int i = 0; i < 3; i++){
				...
				afterCellId =  queryModule.intersectRoomByPoint(session, "Real Data", newPoint);
